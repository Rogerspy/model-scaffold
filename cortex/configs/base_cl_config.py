...
class Config(object):
    ...
    def __init__(self, dataset):
        ...
        self.model_name = 'base_cl_model'
        self.task_names = [f'task_{i+1}' for i in range(13)]
        self.train_path = {
                task_name: os.path.join(
                    base_dir, dataset, 
                    f'clsdcp/tasks/{task_name}/train.json'
                ) for task_name in self.task_names
            }                                                                  
        self.dev_path = {
                task_name: os.path.join(
                    base_dir, dataset, 
                    f'clsdcp/tasks/{task_name}/dev.json'
                ) for task_name in self.task_names
            }                                   
        self.test_path = {
                task_name: os.path.join(
                    base_dir, dataset, 
                    f'clsdcp/tasks/{task_name}/test.json'
                ) for task_name in self.task_names
            }  
        self.replay_path = {
                task_name: os.path.join(
                    base_dir, dataset, 
                    f'clsdcp/replay/{task_name}/train.json'
                ) for task_name in self.task_names
            }                               
