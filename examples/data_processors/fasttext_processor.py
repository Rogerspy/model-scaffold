...
# -*- encoding: utf-8 -*-
'''
@File    :   fasttext_processor.py
@Time    :   2022/05/21 08:22:34
@Author  :   Rogerspy
    ...
    def build_dataset(self, fpath):
        ...
        with open(fpath, encoding='utf8') as f:
            for line in f: 
                ...
                for word in tokens:
                    word_ids.append(
                        self.config.vocab.get(word, self.config.vocab.get(self.config.unk_token))
                    )
...
class DatasetIterater(object):
    ...
    def _to_tensor(self, data):
        x = torch.LongTensor([d[0] for d in data]).to(self.config.device) 
